...
public class TurnTransmitter : MonoBehaviour 
{
    ...
    {
        if (isPlayer) StartCoroutine(playerHand.DrawCardPhase(amount)); else StartCoroutine(enemyHand.DrawCardPhase(amount));
    }
    public void CallPartnerInHandsAndDeck(bool isPlayer) 
    { if (isPlayer) { //List<CardEntity> partner = new List<CardEntity>();//foreach (CardEntity card in playerHand.handList)//    if (card.cardData.abilityAndStatus.ContainsKey(Enums.CardAbility.PartnerSummon))
                      ////        card.cardData.CardCost = 0;
                      StartCoroutine(playerHand.DrawAndSummonPartners());
                  }
        else { //List<CardEntity> partner = new List<CardEntity>();//foreach (CardEntity card in enemyHand.handList)//    if (card.cardData.abilityAndStatus.ContainsKey(Enums.CardAbility.PartnerSummon))//        card.cardData.CardCost = 0;              StartCoroutine(enemyHand.DrawAndSummonPartners());         }     }      private void DrawInnateCards()     {         var innateCards = PlayerBattleDeck.BattleDeck.FindAll(x => x.abilities.Contains(Enums.CardAbility.InnateCard));         foreach (var card in innateCards)         {             StartCoroutine(playerHand.AddDefiniteCardToHand(card, false));             PlayerBattleDeck.BattleDeck.Remove(card);         }         innateCards = PlayerBattleDeck.EnemyBattleDeck.FindAll(x => x.abilities.Contains(Enums.CardAbility.InnateCard));         foreach (var card in innateCards)
        {
